...
KEYCHAIN_TEST_BINARY="${ECP_BIN_DIR}/ecp"
...
$OPENSSL_CLI pkcs12 -legacy -inkey ${EC_CLIENT_KEY} -in ${EC_CLIENT_CERT} -export -out cred.p12 -passin pass:${PASSWORD} -passout pass:${PASSWORD}
security import cred.p12 -P ${PASSWORD} -k ${KEYCHAIN} -A

$OPENSSL_CLI pkcs12 -legacy -inkey ${RSA_CLIENT_KEY} -in ${RSA_CLIENT_CERT} -export -out cred.p12 -passin pass:${PASSWORD} -passout pass:${PASSWORD}
security import cred.p12 -P ${PASSWORD} -k ${KEYCHAIN} -A
  ...
  cat << EOF > ec_certificate_config.json
{
  ...
  "libs": {
    "ecp": "$ECP_BIN_DIR/ecp",
    "ecp_client": "$ECP_BIN_DIR/libecp.dylib",
    "tls_offload": "$ECP_BIN_DIR/libtls_offload.dylib"
  }
}
EOF
}
